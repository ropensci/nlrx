...
nlrx provides two independent ways to run the same parameterization more than once.
They differ in who controls the random seed.

The `nseeds` argument of the simdesign helper functions is the nlrx-managed mechanism.
...
Each run is seeded explicitly, the seed is reported back in the `random-seed` column of the results, and runs can therefore be reproduced.

The `repetition` slot of the experiment is the NetLogo-managed mechanism.
It is handed to BehaviorSpace, which repeats every parameterization that many times.
Because BehaviorSpace would otherwise run all repetitions with the same seed and produce identical results, nlrx omits the `random-seed` variable whenever `repetition > 1`.
NetLogo then seeds each run itself, based on the current date and time.
The repetitions do differ from one another, but their seeds are neither reported nor recoverable, so individual runs cannot be reproduced.
...
Thus we suggest to set the experiment repetition to 1 and instead use the nseeds variable of the desired simdesign to run multiple simulations with different random seeds.
Use `repetition > 1` only if reproducibility of individual runs is not needed.

The two mechanisms should not be combined: setting `repetition > 1` drops the simdesign seeds for all runs, so the seeds generated by `nseeds` have no effect and merely multiply the number of unseeded repetitions.
