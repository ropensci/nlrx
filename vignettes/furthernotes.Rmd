...
The run_nl_all function uses the `future_map_dfr()` function from the [furrr package](https://github.com/futureverse/furrr). The simulations are executed in a nested loop where the outer loop iterates over the random seeds of your simdesign, and the inner loop iterates over the rows of the siminput parameter matrix of your simdesign. These loops can be executed in parallel by setting up an appropriate plan from the [future package](https://github.com/futureverse/future). See examples below for more details on parallelisation on local machines and remote HPC clusters.
