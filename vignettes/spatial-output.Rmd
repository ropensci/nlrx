...
The experiment class will soon be updated to also support capturing link variables.

Basically, you can enter any variable of your model that is listed in turtles-own or patches-own, however if you add variables that contain strings, these strings must not contain any whitespaces or the output data will not be parsed correctly.
...
Using the gganimate package, it is even possible to generate animated plots from this spatial data tibble.
...
p1 <- ggplot(results_spatial_tibble) +
  ...
  geom_point(aes(x = X, y = Y, group=who, color = breed), size=2) +
  scale_fill_manual(values = c("bare" = "darksalmon", "grass" = "chartreuse4")) +
  scale_color_manual(values = c("sheep" = "white", "wolves" = "black")) +
...
gganimate::animate(p1, nframes = max(patches$step), width=400, height=400, fps=4)
