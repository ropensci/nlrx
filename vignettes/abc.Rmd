...
First, because parameterizations depend of results from previous runs, a single calibration can not be parallelized.
Second, `run_nl_dyn()` runs one complete calibration for each random seed of the simdesign, one after another.
Third, the returned tibble contains one row per seed, with the seed in the `seed` column and the result of the respective calibration in the `result` list column.
Note that with `use_seed = TRUE` the seeds of the model runs are chosen by the EasyABC functions; the seeds of the simdesign then only determine how many independent calibrations are run.
...
Because the model is stochastic, the summary statistics of a single model run are noisy.
The `nreplicates` argument of `run_nl_dyn()` simulates each proposed parameterization several times with different random seeds and passes the averaged summary statistics to the algorithm.
With `use_seed = TRUE` the replicate seeds are derived from the seed that EasyABC supplies for the respective step:
...
Each calibration reports its output as a nested tibble, stored in the `result` column of the returned tibble.
The post-processing below inspects one of these calibrations, so we attach the result of the first seed to the nl object.
...
```{r eval=FALSE}
setsim(nl, "simoutput") <- results$result[[1]]
