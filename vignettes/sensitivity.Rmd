...
The variation between those replicated runs is an indicator of the stochasticity effects within the model.
...
Sensitivity analyses typically have many runs that need to be simulated, thus we recommend to parallelize model runs by increasing `threads` (and, for larger designs, distributing runs across cluster nodes; more details on parallelization can be found in the "Advanced configuration" vignette).

```{r eval=FALSE}
results <- run_nl_all(nl, threads = 4)
```
