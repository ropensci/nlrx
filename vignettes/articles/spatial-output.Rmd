...
ggplot() +
  ...
  coord_equal() +
  geom_tile(data=results_unnest_patches, aes(x=pxcor, y=pxcor, fill=factor(pcolor))) +
...
gganimate::animate(p1, nframes = max(results_unnest$`[step]`), width=400, height=400, fps=4)
...
In order to execute th `nl_to_graph()` function, the output results need to be attached to the nl object.
