...
food_dat <- nl_spatial %>% dplyr::filter(food > 0) %>% dplyr::select(patches_x, patches_y, step)
...
nest_dat <- food_dat
nest_dat$patches_x <- 0
nest_dat$patches_y <- 0
...
p1 <- ggplot(food_dat) +
  ...
  geom_point(data=nest_dat, aes(x=patches_x, y=patches_y), color="brown", fill="white", size=30, stroke=2, shape=21) +
...
Again, after attaching the simulation results and postprocessing with get_nl_spatial (note: we set links = TRUE, turtles = TRUE, patches = FALSE) we split the tibble into a turtles tibble and a links tibble by subsetting the group column.
...
nl_spatial <- get_nl_spatial(nl, turtle_coords = "x", format = "tibble", patches = FALSE, links = TRUE, turtles = TRUE)
...
<center>
<img src="networks.gif" align="center" width="75%" />
</center>
...
We use the `graph_from_data_frame()` function from the igraph package to convert our spatial data into an igraph object.
...
## Split data from get_nl_spatial into links and turtles
## Filter to select only last simulation step
## Rename columns (from, to for links; name for turtles)
nl_links <- nl_spatial %>% 
  dplyr::filter(step==max(step)) %>% 
  dplyr::filter(group == "links") %>% 
  dplyr::select(end1, end2) %>% 
  dplyr::rename(from=end1, to=end2)

nl_turtles <- nl_spatial %>%
  dplyr::filter(step==max(step)) %>% 
  dplyr::filter(group == "turtles") %>% 
  dplyr::select(who) %>% 
  dplyr::rename(name=who)

## Load igraph package and convert spatial tibbles to igraph:
library(igraph)
nw <- graph_from_data_frame(nl_links, directed = FALSE, vertices = nl_turtles)
