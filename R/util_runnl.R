...
util_read_write_batch <- function(nl) {
  ...
  if (os == "win") {
    ...
    if (nl_majorversion == 6) {
      ## If NetLogo version is 6, we can just copy the headless bat from the installation folder:
      # Prepare pathes:
      ...
      ## If NetLogo version is lower than 6, we have to write the batchfile manually
      ## as there is none provided within the installation folder:
