...
#' @param nreplicates number of replicated model runs per evaluation
...
util_run_nl_dyn_GenSA <- function(nl,
  ...
  # Derive the replicate seeds once, so that all evaluations use the same seeds
  # (common random numbers, see the "Replicated evaluations" section of run_nl_dyn()):
  seeds <- util_generate_replicate_seeds(seed, nreplicates)
...
#' @param seed vector of model seeds, one per replicated run of this evaluation
#' @param threads number of NetLogo threads used for execution (handled via NetLogo).
...
util_run_nl_dyn_GenSA_fn <- function(param,
  ...
  if(is.function(evalcrit)) {
    ...
    # Select evalcrit metric and reduce it over ticks and replicated runs:
    results <- util_reduce_replicates(results, evalcrit)[[evalcrit]]
  }
...
util_run_nl_dyn_GenAlg <- function(nl,
  ...
  # Get GenAlg object from simdesign:
  ...
  # Call the rbga function from the genalg package:
  results <- genalg::rbga(
...
#' @keywords internal
util_run_nl_dyn_ABCmcmc <- function(nl, seed, threads = 1, nreplicates = 1, ...)
{
...
util_run_nl_dyn_ABCmcmc_fn <- function(param, threads = 1)
{
  ...
  nreplicates <- globals$nreplicates
  ...
  # Derive the seeds of the replicated runs of this evaluation. With use_seed = TRUE the
  # seed is supplied by EasyABC per call, so the replicate seeds follow that seed.
  seeds <- util_generate_replicate_seeds(seed, nreplicates)
  ...
  if(is.function(postpro_function)) {
    ...
    # If no function is provided, the defined metrics columns are reduced over
    # ticks and replicated runs:
    results <- unlist(util_reduce_replicates(results, getexp(nl, "metrics")))
  }
  return(results)
}
